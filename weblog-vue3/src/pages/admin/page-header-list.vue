<template>
  <el-card shadow="never" :body-style="{ padding: '20px' }" class="mb-5 border-1">
    <!-- card body -->
    <el-text class="mx-1 mr-3">路由名称</el-text>
    <el-input v-model="searchRouteName" placeholder="请输入（模糊查询）" class="w-50 mr-5"/>
  ...
  <el-card shadow="never" class="border-1">
    ...
    <el-table :data="tableData" stripe style="width: 100%" class="mt-4" v-loading="tableLoading">
      ...
      <el-table-column prop="bgUrl" label="图片路径" width="180"/>
      <el-table-column label="图片预览" width="180">
        <template #default="scope">
          <el-image style="width: 100px; height: 100px" :src="scope.row.bgUrl"/>
        </template>
      </el-table-column>
      <el-table-column prop="createdAt" label="创建时间" width="180"/>
      <el-table-column label="操作">
        <template #default="scope">
          <el-button type="success" size="small" @click="deleteCategorySubmit(scope.row)">
            <el-icon class="mr-1">
              <Edit/>
            </el-icon>
            编辑
    ...
    <div class="mt-5 flex item-center justify-center">
      <el-pagination v-model:current-page="current" v-model:page-size="size" :page-sizes="[10, 20, 50]"
                     :small="small" :disabled="disabled" background="true"
                     layout="total, sizes, prev, pager, next, jumper"
                     :total="total" @size-change="handleSizeChange" @current-change="getTableData"/>
    </div>
  </el-card>

  <el-dialog v-model="isAddCatagoryDialogShow" title="新增配置" width="30%" :show-close="false" draggable>
...
import {savePageHeader, getPageHeaderList, deletePageHeader} from '@/api/admin/pageHeader'
import {showMessage} from '@/composables/util'
import moment from 'moment';
import {Search, RefreshRight, Delete, Edit, Plus} from '@element-plus/icons-vue'
...
const deleteCategorySubmit = (row) => {
  ElMessageBox.confirm(
      ...
      .then(() => {
        deletePageHeader(row.id).then((e) => {
          if (e.success == true) {
